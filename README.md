...
把游戏 ICON 原图批量导出成**各硬核渠道(OPPO / vivo / 华为 / 小米 / 荣耀)所需的规格尺寸**:直角 216 / 256 / 258 / 320 / 512 + 圆角 222 的 PNG,按「游戏名-尺寸 类型.png」命名,**全程无损编码**。
...
- 导出**全程无损**(像素零失真);颜色简单的图标自动走调色板,体积更小
- 结果卡片可查看大图、打开所在目录
- 自动适配系统深色 / 浅色主题
...
- 无损编码在主进程用纯 Node(zlib)实现:颜色数 ≤256 的图自动走 8-bit 调色板(精确保留、体积最小),全不透明自动去掉 alpha 通道,每行在 None/Sub/Up/Avg/Paeth 五种过滤器里选最优再最高级 deflate——像素零失真;
