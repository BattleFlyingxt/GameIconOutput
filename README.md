...
- 降色采用 **TinyPNG / pngquant 同款思路**:感知色彩空间(γ≈2.2)+ 亮度加权距离、K-means 调色板精修、Floyd–Steinberg 抖动(只抖颜色不抖透明,圆角边缘干净)、调色板瘦身、deflate 多策略择优 —— 同体积下画质明显更好、渐变不色带
- 结果卡片可查看大图、打开所在目录
- 自动适配系统深色 / 浅色主题
...
- 自适应编码在主进程用纯 Node(zlib)实现:先无损(颜色数 ≤256 走调色板并按色数压位深 1/2/4/8,全不透明去 alpha,每行在 None/Sub/Up/Avg/Paeth 里选最优再最高级 deflate);无损超出 100KB 才做 TinyPNG 式按需降色 —— 感知空间(γ≈2.2)4D 中位切分初始化 + K-means 精修,每档先试抖动(Floyd–Steinberg,仅 RGB)再试不抖动,从 256 色逐级压到体积达标即停,调色板瘦身 + deflate 多策略择优;
