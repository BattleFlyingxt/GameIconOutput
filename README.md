...
- 量化采用 **TinyPNG / pngquant 同款思路**:感知色彩空间(γ≈2.2)+ 亮度加权距离、K-means 调色板精修、Floyd–Steinberg 抖动(只抖颜色不抖透明,圆角边缘干净)、调色板瘦身、deflate 多策略择优 —— 同体积下画质明显更好、渐变不色带
- **窗口标题与页头显示当前版本号**(如「游戏图标导出 v1.0.6」),发新版本时自动跟随 package.json
- 结果卡片可查看大图、打开所在目录
- 自动适配系统深色 / 浅色主题
...
- 编码在主进程用纯 Node(zlib)实现,策略与 tinypng.com 一致:颜色数 ≤256 走精确调色板(按色数压位深 1/2/4/8,全不透明去 alpha);颜色数 >256 一律感知量化 —— 感知空间(γ≈2.2)4D 中位切分初始化 + K-means 精修,每档先试抖动(Floyd–Steinberg,仅 RGB)再试不抖动,从 256 色逐级压到 ≤100KB 即停,调色板瘦身 + deflate 多策略择优;不再做无损 RGB/RGBA 真彩输出;
